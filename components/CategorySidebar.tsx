...
const FORUM_THREADS = [
  { 
    id: 't1',
    title: "With so many new AI Note Takers - what's your favorite and why?", 
    author: "Gabe Perez",
    views: '1.6K', 
    time: '8MO AGO' 
  },
  { 
    id: 't2',
    title: "Any AI notetakers that respect data privacy and local-first storage?", 
    views: '840', 
    time: '2MO AGO' 
  },
  { 
    id: 't3',
    title: "Transcribing sermons with AI: The best tools for high accuracy", 
    views: '2.1K', 
    time: '1Y AGO' 
  },
  {
    id: 't4',
    title: "Integrating Fathom with non-standard CRMs for custom workflows",
    views: '420',
    time: '3W AGO'
  }
];
...
const PRODUCTIVITY_LINKS = [
  "AI notetakers", "AI Presentation Software", "Ad blockers", "App switcher", 
  "Content Management Systems", "Calendar apps", "Compliance software", 
  "Customer support tools", "E-signature apps", "Email clients", 
  "File storage and sharing apps", "Hiring software", "Knowledge base software", 
  "Legal services", "Meeting software", "Note and writing apps", "PDF Editor", 
  "Password managers", "Presentation Software", "Product demo", 
  "Project management software", "Resume tools", "Scheduling software", 
  "Screenshots and screen recording apps", "Search"
];
...
const CategorySidebar: React.FC<CategorySidebarProps> = ({ activeCategory = "AI notetakers", onCategorySelect }) => {
  return (
    <aside className="hidden lg:block space-y-10 sticky top-24 h-fit pb-12 overflow-y-auto max-h-[calc(100vh-6rem)] custom-scrollbar">
            ...
            <div key={thread.id} className="group cursor-pointer">
              ...
              <div className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">
                ...
                {thread.author && (
                  <>
                    <span className="text-gray-200">•</span>
                    <span className="text-emerald-700/60 font-black">BY {thread.author.toUpperCase()}</span>
                  </>
                )}
              </div>
            </div>
      ...
      {/* Contextual Ads / Stats */}
      <section className="bg-white border border-gray-100 rounded-[2rem] p-6 shadow-sm">
        <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Niche Stats</h4>
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between text-xs font-bold">
            <span className="text-gray-500 font-medium">Makers Active</span>
            <span className="text-gray-900">128</span>
          </div>
          <div className="flex items-center justify-between text-xs font-bold">
            <span className="text-gray-500 font-medium">Monthly Reviews</span>
            <span className="text-gray-900">1.2K+</span>
          </div>
        </div>
      </section>

      {/* More in Productivity Section */}
