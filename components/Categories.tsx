  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 py-12">
        <button onClick={onBack} className="flex items-center gap-2 text-gray-400 hover:text-primary transition-colors mb-12 group font-black uppercase tracking-widest text-[10px]">
        ...
        <div className="text-center mb-16">
          ...
          <h1 className="text-5xl sm:text-7xl font-serif font-bold text-primary tracking-tighter mb-6">Product Categories</h1>
        ...
        <div className="sticky top-[4.1rem] z-40 bg-white/80 backdrop-blur-md py-4 mb-16 border-b border-gray-50">
          <div className="max-w-xl mx-auto mb-6">
            <div className="relative group">
              ...
              <input 
                ...
                className="w-full pl-14 pr-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-md font-medium outline-none focus:bg-white focus:border-primary focus:ring-4 focus:ring-emerald-900/5 transition-all shadow-sm"
              />
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 overflow-x-auto pb-2 scrollbar-hide">
            {Object.keys(groupedCategories).map((group) => (
              <button key={group} onClick={() => scrollToSection(group)} className="flex items-center gap-2 px-4 py-2 bg-gray-50 border border-gray-100 rounded-full text-xs font-black text-gray-500 uppercase tracking-widest hover:bg-primary-light hover:text-primary transition-all whitespace-nowrap">
                {group}
              </button>
            ))}
          </div>
          ...
          {(Object.entries(groupedCategories) as [string, Category[]][]).map(([group, items]) => (
            <section key={group} id={group} className="scroll-mt-20">
              <div className="flex items-center gap-3 mb-10 border-b border-gray-100 pb-6">
                <h2 className="text-3xl font-serif font-bold text-primary">{group}</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {items.map((item) => {
                  const Icon = ICON_MAP[item.icon_name] || Hash;
                  return (
                    <button key={item.id} onClick={() => onCategorySelect(item.name)} className="flex flex-col p-6 bg-white border border-gray-100 rounded-2xl hover:border-primary-light hover:shadow-xl transition-all text-left group">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="p-2 bg-primary-light text-primary rounded-lg"><Icon className="w-5 h-5" /></div>
                          <h3 className="text-base font-bold text-gray-900 group-hover:text-primary transition-colors">{item.name}</h3>
                        </div>
