...
const AdminPanel: React.FC<AdminPanelProps> = ({ user: initialUser, onBack, onRefresh }) => {
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-white animate-in fade-in duration-500">
        <div className="max-w-md w-full text-center">
          ...
          <button
            ...
            className="w-full px-10 py-5 bg-primary-dark text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-primary transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3"
          >
            Return to Login
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-20 bg-[#042119] bg-[radial-gradient(circle_at_top_left,rgba(6,78,59,0.4),transparent)]">
        <div className="w-full max-w-md bg-white rounded-[3.5rem] p-10 sm:p-14 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.6)] border border-emerald-900/10 animate-in fade-in zoom-in-95 duration-700 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-40 h-40 bg-primary-light rounded-full -mr-20 -mt-20 opacity-40 blur-3xl" />

          <div className="flex flex-col items-center mb-12 text-center relative z-10">
            <div className="w-24 h-24 bg-primary-dark rounded-[2.5rem] flex items-center justify-center text-white mb-8 shadow-2xl shadow-emerald-900/40 transform -rotate-3 hover:rotate-0 transition-transform duration-500">
              <KeyRound className="w-12 h-12" />
            </div>
            <h1 className="text-3xl font-serif font-bold text-primary mb-2">Muslim Hunt Admin</h1>
          ...
          <form onSubmit={handleAdminLogin} className="space-y-5 relative z-10">
            <div className="space-y-1">
              <label className="text-[10px] font-black text-primary/40 uppercase tracking-[0.2em] px-1">Identity</label>
              <div className="relative group">
                <Mail className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-primary/30 group-focus-within:text-primary transition-colors" />
                <input
                  type="email" placeholder="zeirislam@gmail.com" value={email} onChange={(e) => setEmail(e.target.value)} required
                  className="w-full pl-12 pr-6 py-4 bg-primary-light/30 border border-transparent focus:bg-white focus:border-primary rounded-2xl outline-none font-bold text-gray-900 transition-all shadow-inner placeholder:text-gray-300"
                />
              </div>
            </div>

            <div className="space-y-1">
              <label className="text-[10px] font-black text-primary/40 uppercase tracking-[0.2em] px-1">Security Key</label>
              <div className="relative group">
                <Lock className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-primary/30 group-focus-within:text-primary transition-colors" />
                <input
            ...
            <button
              ...
              className="w-full bg-primary-dark hover:bg-primary text-white font-black py-5 rounded-[1.5rem] transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50 mt-4 group shadow-xl shadow-emerald-900/20"
            >
          ...
          <button onClick={onBack} className="w-full mt-10 py-2 text-[10px] font-black text-gray-400 hover:text-primary uppercase tracking-[0.3em] transition-colors flex items-center justify-center gap-2 group">
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 py-12 px-4 sm:px-8 animate-in fade-in duration-500">
      ...
      <div className="max-w-7xl mx-auto bg-gradient-to-br from-emerald-50/40 via-white to-white border-4 border-primary-light/50 rounded-[3.5rem] p-8 md:p-12 shadow-[0_32px_64px_-12px_rgba(6,78,59,0.1)]">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
          <div className="space-y-3">
            ...
            <h1 className="text-5xl font-serif font-bold text-primary tracking-tight leading-none">Submission Review</h1>
          ...
          <div className="flex items-center gap-4">
            <div className="bg-white border border-gray-100 rounded-[2rem] px-10 py-5 shadow-sm text-center min-w-[180px] border-b-4 border-b-emerald-800">
              ...
              <span className="text-4xl font-serif font-bold text-primary leading-none">{pendingProducts.length}</span>
            </div>
            <div className="flex flex-col gap-2">
              <button
                onClick={fetchPending}
                className="p-4 bg-white border border-gray-100 rounded-2xl text-gray-400 hover:text-primary transition-all shadow-sm active:scale-90 group"
                title="Refresh Sync"
              >
        ...
        <div className="bg-white border border-primary-light rounded-t-[3rem] p-6 sm:p-10 flex flex-col md:flex-row items-center gap-6">
          <div className="relative flex-1 w-full">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-300" />
            <input
              type="text" placeholder="Search pending products..."
              value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-16 pr-8 py-5 bg-gray-50 border border-transparent focus:bg-white focus:border-primary rounded-[2rem] outline-none transition-all font-bold text-gray-900 shadow-inner"
            />
          </div>
        ...
        <div className="bg-white border-x border-b border-primary-light rounded-b-[3rem] overflow-hidden shadow-2xl shadow-emerald-900/5">
                ...
                {loading && pendingProducts.length === 0 ? (
                  ...
                  <tr>
                    <td colSpan={5} className="px-12 py-48 text-center">
                      ...
                      <h2 className="text-3xl font-serif font-bold text-primary mb-3 tracking-tight">Queue All Clear</h2>
                      <p className="text-gray-400 font-medium italic">All pending submissions have been processed.</p>
                    </td>
                  </tr>
                ) : (
                  filteredQueue.map((p) => (
                    <tr key={p.id} className="group hover:bg-primary-light/10 transition-colors">
                      <td className="px-12 py-10">
                        <div className="flex items-center gap-6">
                          ...
                          <div className="min-w-0">
                            <p className="text-lg font-black text-gray-900 leading-none mb-2 truncate group-hover:text-primary transition-colors">{p.name}</p>
                            <div className="flex items-center gap-2 text-xs text-gray-400 font-bold truncate max-w-[280px]">
                              <Hash className="w-3.5 h-3.5 text-primary opacity-30" />
                              {p.tagline}
                            </div>
                          </div>
                        </div>
                      ...
                      <td className="px-12 py-10 text-right">
                        <div className="flex items-center justify-end gap-4">
                          <a
                            ...
                            className="p-4 text-emerald-700/60 hover:text-primary hover:bg-primary-light rounded-2xl transition-all border border-primary-light/50 active:scale-95"
                          >
      ...
      <div className="mt-12 text-center">
        <button onClick={onBack} className="text-gray-400 hover:text-primary transition-colors font-black uppercase tracking-[0.2em] text-[10px] flex items-center justify-center gap-2 mx-auto group">
