...
import { 
  ...
  Triangle, Menu, Layout, Hash, ShieldCheck, Calendar, Trophy, CheckCircle2
} from 'lucide-react';
                ...
                <div className="relative flex items-center" ref={notificationDropdownRef}>
                  ...
                  {showNotificationDropdown && (
                    <div className="absolute top-full right-0 mt-2 w-80 bg-white border border-gray-100 shadow-2xl rounded-2xl py-6 z-[110] animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="space-y-1 mb-6 max-h-[320px] overflow-y-auto">
                        {notifications.length === 0 ? (
                          <p className="text-[13px] text-gray-400 italic text-center py-10 px-6">No unread notifications!</p>
                        ) : (
                          notifications.slice(0, 2).map((n) => (
                            <div key={n.id} onClick={() => handleNavigate(View.NOTIFICATIONS)} className={`flex gap-4 px-6 py-4 hover:bg-emerald-50/50 cursor-pointer transition-all border-b border-gray-50 last:border-0 ${!n.is_read ? 'bg-emerald-50/20' : ''}`}>
                              <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border shadow-sm ${n.is_read ? 'bg-gray-50 border-gray-100 text-gray-400' : 'bg-emerald-50 border-emerald-100 text-emerald-900'}`}>
                                {n.type === 'upvote' ? <Triangle className="w-5 h-5 fill-current" /> : 
                                 ...
                                 n.type === 'submission' ? <ShieldCheck className="w-5 h-5" /> :
                      ...
                      <div className="px-6">
                        <button onClick={() => handleNavigate(View.NOTIFICATIONS)} className="w-full py-3 bg-white border border-gray-100 rounded-xl text-xs font-black text-emerald-800 hover:bg-emerald-50 transition-all uppercase tracking-widest shadow-sm">View all discussions</button>
                      </div>
                    </div>
                  )}
                </div>
