...
const App: React.FC = () => {
  ...
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({ today: false, yesterday: false });
  ...
  const fetchProducts = async () => {
    ...
    // Check if initial URL points to a product
    const path = window.location.pathname;
    if (path.startsWith('/products/')) {
      const slug = path.split('/products/')[1];
      const prod = findProductBySlug(prods, slug);
      if (prod) {
  ...
  return (
    <div className={`min-h-screen bg-white selection:bg-emerald-100 selection:text-emerald-900`}>
      ...
      <main className={`${view === View.DETAIL ? 'max-w-[1100px]' : 'max-w-[1100px]'} mx-auto px-4 lg:px-6 py-8`}>
        {view === View.HOME && (
                      ...
                      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden mb-6">
                        {section.data.map((p, i) => (
                          <ProductCard 
                            ...
                            onClick={(prod) => { setSelectedProduct(prod); setView(View.DETAIL); safeHistory.push(`/products/${slugify(prod.name)}`); }} 
                            onCommentClick={() => { setSelectedProduct(p); setShouldScrollToComments(true); setView(View.DETAIL); safeHistory.push(`/products/${slugify(p.name)}`); }} 
                          />
                        ))}
                      </div>
                    </section>
